...
class Actes
{
    ...
    public function getActesList(
        ...
        int $limit,
        int $offset,
        string $minSubmissionDate,
        string $maxSubmissionDate
    ): stdClass {
        $actesListQuery = new ActeListQuery();
        $actesListQuery->statusId = $statusId;
        $actesListQuery->limit = $limit;
        $actesListQuery->offset = $offset;
        $actesListQuery->minDate = $minSubmissionDate;
        $actesListQuery->maxDate = $maxSubmissionDate;
        return json_decode($this->client->get(self::LIST_ACTES_API, $actesListQuery), false, 512, JSON_THROW_ON_ERROR);
    }
    ...
    public function getFileList(string $transactionId): array
    {
        ...
        return json_decode($fetched_files, true, 512, JSON_THROW_ON_ERROR);
    }
